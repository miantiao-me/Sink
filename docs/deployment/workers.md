...
4. (_Optional_) For OpenGraph image upload, create an [R2 bucket](https://developers.cloudflare.com/r2/) named `xpertl` (or run `wrangler r2 bucket create xpertl`). If you don't need this feature, comment out the `r2_buckets` section in `wrangler.jsonc`.
5. Create a project in [Cloudflare Workers](https://developers.cloudflare.com/workers/).
6. Select the `Xpertl` repository and use the following build and deploy commands:
...
9. Enable Analytics Engine. In **Workers & Pages**, go to **Account details** in the right panel, locate **Analytics Engine**, and click **Set up** to enable the free tier. Name them `xpertl` and `ANALYTICS`, or else overwrite it with `NUXT_DATASET` and update your `wrangler.jsonc` accordingly
